...
(function ($) {
    function mainFm(selector, params) {
        var defaults = $.extend({}, {currentPage: " ", animationSpeed: 1E3, slideshowSpeed: 5E3}, params);
        var self = this;
        self.layoutHorizontal = $("body").hasClass("horizontal_layout");
        if ($("body").hasClass("high_mobile_performance") && window.innerWidth < 992)$("body").addClass("horizontal_layout");
        self.onePage = $("body").hasClass("not_onepage_ver") ? false : true;
        self.screenWidth = window.innerWidth;
        self.alignPgHor = self.scrollHorizontal = scrollHorizontal = self.onePage ? $("body").hasClass("horizontal_layout") :
            false;
        ...
        768) && (self.stageWidth === 1024 || self.stageHeight === 1024);
        ...
        self.azh = undefined;
        if (!self.onePage)$(".contentWrapper").attr("data-id", self.homePage);
        ...
        self.supportScrollBar =
            true;
        if (!self.layoutHorizontal && $.browser.mozilla)self.supportScrollBar = false;
        self.singleBg = true;
        if (self.onePage) {
        ...
        $("body").find(".gallery_autoThumbnail").each(function () {
            $(this).find(".carousel_preview").attr("id",
                "prjId" + prId);
            if ($(this).find(".carousel_container").length == 0) {
                ...
                $(this).find(".carousel_preview").find(".carousel_item").each(function () {
                    ...
                    carThu.children(":last-child").attr("data-preview",
                        "#prjId" + prId + "-" + thuNo);
                    thuNo++
                })
            }
        ...
        if (self.HideUnuse)for (var bb = 0; bb < self.conArry.length; bb++)self.conArry[bb].css({"visibility": "hidden"});
        self.navArry = [];
        for (var ab = 0; ab < self.conArry.length; ab++) {
            var n_spt = self.conArry[ab];
            if (n_spt.attr("data-id") !== undefined)self.navArry.push(self.conArry[ab])
        }
        ...
        self.previewSetting();
        if (self.alignPgHor)if (!isTouch)self.addScrollbar(); else $(".m-Scrollbar").each(function () {
            ...
            $(this).scroll(function () {
                ...
                srcInt = setInterval(function () {
                    ...
                    if (-mcScr.scrollTop() < -150) {
                        for (ik = 0; ik < self.curPageShow.data("fms").length; ik++)if (self.curPageShow.data("fms")[ik].data("loaded"))self.curPageShow.data("fms")[ik].fmMainSlider("pause_slideshow");
        ...
        }); else if (isTouch)self.htmlBody.css({"-webkit-overflow-scrolling": "touch"});
        if (!isTouch && !self.IEbrowser && self.supportScrollBar)self.nicScrl = $("html").niceScroll({
            zindex: 922E5,
            styler: "fb",
            cursorborder: "0px",
            scrollspeed: 100,
        ...
        self.$html = $("html");
        self.$window = $("body");
        $(".contactPage .contactPage_content").css({
            "min-height": self.stageHeight - self.ContPgTopSpace,
            "margin-top": self.ContPgTopSpace
        });
        ...
        $(".fmSlider").each(function () {
            var sel = $(this);
            $("html").keydown(function (e) {
                if (sel.data("url_") === self.url)if (sel.data("loaded")) {
                    if (e.keyCode === 39) {
        ...
        for (var ab = 0; ab < self.conArry.length; ab++) {
            ...
            main_holder.find("[data-animated-in]").each(function () {
                ...
                if (aniMc.find(".graph_container").length > 0)aniMc.data("isAniObj", true);
                if (aniMc.find(".animate_counter").length > 0)aniMc.data("isAniObj", true);
                if (aniMc.find(".big_video").length > 0)aniMc.data("isVidObj", true)
            });
        ...
        $("body").find(".portfolioPage").each(function () {
            ...
            main_holder.find("[data-animated-in]").each(function () {
                ...
                if (self.alignPgHor)$(this).css({"visibility": "visible"})
            });
            main_holder.find("[data-animated-innerContent]").each(function () {
                $(this).children().each(function () {
                    $(this).data("isMasonry", true);
        ...
        if (isTouch)$("body").find(".mainContent .addVideo.backGroundVideo").each(function () {
            $(this).data("inMain", true)
        });
        if (isTouch)$(".nav a, .footer_close, .btn-navbar").click(function () {
        ...
        try {
            $("body").find(".animate_counter, .knob").each(function (i) {
                ...
                if (!selK.hasClass("animate_counter"))if (selK.hasClass("knob"))selK.val(selK.attr("data-value")).trigger("change");
                self.knobAni = true
            })
        } catch (e) {
            self.knobAni = false
        ...
        $(".preloadimages_inline img").each(function () {
            var th = $(this);
            var img;
            if (th.hasClass("cssBackground"))img = retinaDevice ? $(this).attr("data-src-2x") : $(this).attr("data-src"); else img = window.innerWidth > 767 ? $(this).attr("data-src") : $(this).attr("data-src-small") ? $(this).attr("data-src-small") : $(this).attr("data-src");
            th.attr("data-src", img);
            th.addClass("preload");
            self.preloadImages.push(th)
        });
        self.imgFinished = 0;
        if (self.preloadImages.length > 0)self.intImgLoad(self.preloadImages[self.imgFinished]); else siteStartOpen = true;
    ...
    mainFm.prototype = {
        initialize: function () {
            ...
            if (isTouch)$("html , body").css({"overflow": "auto"});
            $(".isotope_option").show();
            ...
            try {
                ...
                $(".lightStyle, .inverseStyle, .contentWrapper").bind("click",
                    function () {
                    })
            } catch (e) {
            }
            ...
            $(".header .nav li").each(function () {
                ...
                slf.bind("click", function () {
                    ...
                    if ($(this).attr("href") && $(this).attr("href") !== "undefined" && uul.charAt(0) !== "#") {
                        ...
                        if (cssAnimate)self.headerFad.delay(100)[animateSyntax]({"opacity": 1}, 500, "easeInOutQuart", function () {
                            if (trg !== undefined)window.open(uul, trg); else window.location.href = uul
                        }); else self.headerFad.delay(100).animate({"opacity": 1}, 500, "easeInOutQuart", function () {
                            if (trg !== undefined)window.open(uul,
                                trg); else window.location.href = uul
                        });
                        return false
                    }
                })
            ...
            $(".menu_link").each(function () {
                ...
                qq++;
                if (slf.attr("href") === "" ||
                    slf.attr("href") === undefined)return;
                slf.bind("click", function () {
                    self.removeAutoposition = false;
                    var gg = String($(this).attr("href")).split("#");
                    if (gg[1] === self.url)self.page_position()
            ...
            self.homePg = self.homePage ===
            "" ? self.menuList[0].substr(1, self.menuList[0].length) : self.homePage;
            ...
            self.page_dimension();
            if (!self.alignPgHor)for (var ab = 0; ab < self.conArry.length; ab++) {
            ...
            $(".isotope_items").show();
            if (self.headerFad)if (cssAnimate)self.headerFad.delay(100)[animateSyntax]({"opacity": 0}, 500, "easeInOutQuart", function () {
                self.headerFad.find(".loading_2x").remove();
                self.headerFad.css({"width": "0px", "height": "0px"})
            }); else self.headerFad.delay(100).animate({"opacity": 0},
            ...
            $(".previousPage, .nextPage").bind("click", function () {
                if ($(this).data("url") && $(this).data("url") !== "undefined") {
                    self.removeAutoposition = false;
                    if ($(this).data("url") !== self.url)window.location.href = "#" + $(this).data("url"); else self.page_position();
                    if ($('.nav a[href$="#' + $(this).data("url") + '"]').length > 0) {
                        $(".nav li a").removeClass("active");
                        $('.nav a[href$="#' + $(this).data("url") +
                        '"]').addClass("active")
                    }
                }
            });
            if (isTouch)$(".fadeAfterLoad").css({"display": "block"}); else $(".fadeAfterLoad").delay(200).fadeIn(300);
            ...
            var chkInt = setInterval(function () {
                ...
                self.headHig = self.stageWidth > 991 ? self.stageWidth < 1025 || !self.headMeuTyp1 ?
                    pageHeaderHeight_mini : pageHeaderHeight : self.alignPgHor ? 0 : pageHeaderHeight_mini;
            ...
            if (self.superSlider) {
                ...
                if (!supersizedOnBody) {
                    ...
                    $(".supersized-nav").show();
                    api.min_thumb()
                } else if ($.supersized.vars.is_paused)api.playToggle()
            }
            if (self.rsSlider) {
                rsSliderInit();
                apiRS.revpause()
            ...
            img.bind("load", function () {
                if (self.imgFinished < self.preloadImages.length - 1) {
                    ...
                    var percent = Math.round(self.imgFinished /
                    self.preloadImages.length * 100);

                    $(".loading_2x .text").html(Math.round(self.imgFinished /
                    self.preloadImages.length * 100) + "%")
            ...
            var scrpyMc = [];
            for (var ab = 0; ab < self.conArry.length; ab++)scrpyMc.push(self.conArry[ab]);
            ...
            $(window).scroll(function () {
                ...
                self.scrIntr = setInterval(function () {
                    ...
                    if (!self.alignPgHor) {
                        if (!self.miniMenu)if (self.stageWidth > 1024)if (self.scrollPos >
                            150 && !self.headerMc.hasClass("menuType1"))self.headerMc.addClass("mini"); else self.headerMc.removeClass("mini"); else self.headerMc.addClass("mini");
                        ...
                        if (self.superSlider && !supersizedOnBody)if (self.scrollPos < 250) {
                            if ($.supersized.vars.is_paused)setTimeout(function () {
                                    api.playToggle()
                                },
                                1E3)
                        } else if (!$.supersized.vars.is_paused)setTimeout(function () {
                            api.playToggle()
                        }, 1E3);
                        ...
                        if (self.scrollPos > self.winHeight + 50) {
                            if (!supersizedOnBody)$(".supersized_gallery").css({
                                "visibility": "hidden",
                                "display": "none"
                            });
                        ...
                        if (self.scrollPos > 240)self.pgUp.show(); else self.pgUp.hide();
                        try {
                            var scrpyPos = [];
                            for (var ab = 0; ab < self.conArry.length; ab++)scrpyPos.push(self.conArry[ab].position().top);
                            var ii = scrpyMc.length - 1;
                            var isY = false;
                            self.sUrl = "!home";
                            self.curScrlPgMc = scrpyMc[0];
                            var scrPP = 0;
                            if (self.enableAutoposition)scrPP = self.scrollPos + self.headHig + self.winHeight /
                            ...
                            if (self.HideUnuse)for (var bb = 0; bb < self.conArry.length; bb++)if (ii + 1 !== bb && ii - 1 !== bb && ii !== bb || ii + 1 === self.conArry.length + 1) {
                                if (self.conArry[bb].css("visibility") !== "hidden")self.conArry[bb].css({"visibility": "hidden"})
                            } else if (self.conArry[bb].css("visibility") !==
                                "visible")self.conArry[bb].css({"visibility": "visible"});
                            if (self.scrspy_curPg !== self.sUrl && !self.scrollHorizontal) {
                                ...
                                if (!self.enableLowPerformance && self.azh !== undefined) {
                                    ...
                                    if (self.scrollPos < 250)if (!self.alignPgHor && self.onePage && !isMobileChk)window.location.href = "#" + self.homePage
                                }
                                self.azh = self.scrspy_curPg
                            }
                            if (self.enableLowPerformance && !self.scrollHorizontal)if (self.url !== self.sUrl && self.removeAutoposition && self.onePage)window.location.href =
                                "#" + self.sUrl;
                            clearInterval(self.clearInter3);
                            self.clearInter3 = setInterval(function () {
                                clearInterval(self.clearInter3);
                                if (self.curScrlPgMc.position().top < self.scrollPos)for (var ik = 0; ik < self.curScrlPgMc.data("fms").length; ik++)if (self.curScrlPgMc.data("fms")[ik].data("loaded"))self.curScrlPgMc.data("fms")[ik].fmMainSlider("pause_slideshow")
                            }, 500);
                    ...
                    } else if (self.scrollPos > 240)self.pgUp.show();
                    else {
                        self.pgUp.hide();
                        if (self.onePage && !isMobileChk && !self.scrollHorizontal)window.location.href = "" + self.homePage
                    }
                }, 70)
            })
        }, scroll_by: function (pixels) {
            var self = this;
            if (self.alignPgHor && !isTouch)try {
                if (self.alignPgHor)if (self.curP.hasClass("m-Scrollbar")) {
                    var selg = $(this);
            ...
            var hold;
            if (obj.parent().parent().parent().parent().hasClass("projImgs") || obj.hasClass("resize_align"))if (obj.hasClass("resize_align"))hold = obj.parent(); else hold = obj.parent().parent().parent().parent(); else return;
            ...
            function appy_resizeImg(obj, wid, hig) {
                var iw = wid, ih = hig, ww = hold.width(), wh = hold.height(), rw = wh / ww, ri = ih / iw, tp = 0, lp = 0, newWidth, newHeight, newLeft, newTop, properties;
                if (obj.hasClass("resize_align") && !obj.hasClass("fitInside")) {
                    obj.css({"margin-left": "0px"});
                    var rezr = hold.width() < hold.height() ? rw < ri : rw >
                    ri;
                    newWidth = ww;
                    newHeight = ww * ri;
                    if (rezr)lp = (ww - newWidth) / 2;
            ...
            var self = this;
            if (!self.IEbrowser)$(".isotope_items .item a .img_text").css("visibility", "visible");
            ...
            $("body").find(".flexslider").each(function () {
                if (!$(this).hasClass("flexSlideshow_twitter"))try {
                    if (!$(this).data("loaded")) {
                        $(this).data("loaded",
                            true);
                        var aniTyp = $(this).hasClass("slideAnimation") ? "slide" : "fade";
                        var tim_ = $(this).attr("data-slidetime") ? Math.abs($(this).attr("data-slidetime")) : 5E3;
                        $(this).find("a.lazyload").each(function () {
                            self.lazyLoadInt($(this))
                        });
                        if (aniTyp === "slide")$(this).find("li").each(function (i) {
                    ...
                    $(this).click(function () {
                        ...
                        $(btn.attr("data-close")).each(function () {
                            ...
                            if (!self.isHide) {
                                ...
                                if (cssAnimate)mc[animateSyntax]({"height": "0px"}, 500, "easeInOutQuart"); else mc.animate({"height": "0px"}, 500, "easeInOutQuart");
                                removeTopSpc = true;
                                headerClose = true
                            } else {
                                if (self.stageWidth > 991)$(".header_content").css({"display": "block"});
                            ...
                            if (!self.alignPgHor)setTimeout(function () {
                                self.page_setup()
                            }, 500)
                        });
            ...
            if (!headerClose && !triggerIt) {
                ...
                if (self.stageWidth > 991) {
                    ...
                    $(".header_content ul li ul li").css({
                        "opacity": 1,
                        "bottom": 0
                    });
                    removeTopSpc = true
            ...
            if (self.alignPgHor) {
                $("body").find(".contentWrapper").each(function () {
                    ...
                    if (!lowResDesktop)mcc.css({"height": self.StgHig}); else mcc.css({"height": "auto"})
                });
                if (self.scrollHorizontal)$(".mainContent").css({"width": self.numCon * self.winWidth + 50 + "px"}); else $(".mainContent").css({
                    "width": self.winWidth,
                    "overflow-x": "hidden"
                });
                if (!lowResDesktop && self.scrollHorizontal)$(".mainContent").css({
                    "height": self.StgHig,
                    "overflow-y": "hidden"
                }); else $(".mainContent").css({"height": "auto", "overflow-y": "auto"})
            } else {
            ...
            $("body").find(".fullHeight").each(function () {
                var se2 = $(this);
                if (self.stageWidth > 991)if (self.alignPgHor)se2.css({"min-height": self.StgHig}); else if (se2.hasClass("fixed"))se2.css({"min-height": self.StgHig}); else se2.css({"min-height": self.StgHig - pageHeaderHeight}); else se2.css({"min-height": "none"});
                if (se2.hasClass("fullResponse")) {
                    ...
                    if (se2.hasClass("fullscreenVideo"))se2.css({
                        "min-height": self.StgHig - dumY,
                        "min-width": "100%"
                    });
                    se2.find(".video_content.fullscreenVideo, .fullscreenVideo ").css({
                        "min-height": self.StgHig - dumY,
                        "min-width": "100%"
                    })
                } else if (self.mobile) {
            ...
            if ($('.nav a[href$="#' + ele.attr("data-id") + '"]').length > 0 || !self.onePage) {
                ...
                if (self.onePage) {
                    if (!$('.nav a[href$="#' + iid + '"]').parent().parent().hasClass("nav"))$('.nav a[href$="#' + iid + '"]').parent().parent().parent().children(":first-child").addClass("active");
                    $('.nav a[href$="#' + iid + '"]').addClass("active")
                } else {
                    if (!$('.nav a[href$="' +
                        self.homePage + '"]').parent().parent().hasClass("nav"))$('.nav a[href$="' + self.homePage + '"]').parent().parent().parent().children(":first-child").addClass("active");
            ...
            ele.find(".flexslider").each(function () {
                try {
                    if ($(this).data("slid") !== undefined)$(this).data("slid").windowRez()
                } catch (e) {
                }
            });
            ...
            ele.find(".fullScreenGallery_thumbnails").each(function () {
                ...
                if (mc.width() < 15)sc.text("OPEN"); else sc.text("CLOSE");
                if (self.alignPgHor && !isTouch) {
                    ele.mCustomScrollbar("update");
                    ele.mCustomScrollbar("scrollTo", "top")
                }
            });
            if (lowResDesktop)ele.find("img.resize_align").each(function () {
                self.resizeImg($(this))
            });
            if (!ele.hasClass("portfolioPage"))setTimeout(function () {
                self.videoRest();
                self.scroll_update()
            }, 1E3);
            if (!self.alignPgHor) {
                for (var ab = 0; ab < self.conArry.length; ab++)if (self.conArry[ab].attr("data-id") !== ele.attr("data-id"))self.conArry[ab].find(".fmSlider").each(function () {
                    if ($(this).data("loaded"))$(this).fmMainSlider("pause_slideshow")
                });
                if (!self.enableLowPerformance)if (self.enableAutoposition && !self.temprDisableAutoposition) {
                    if (ele.hasClass("autoPosition") && self.onePage) {
                        self.autoPagePos(ele);
                        if (!isTouch && !self.IEbrowser && self.supportScrollBar)self.nicScrl.cancelScroll();
                        window.location.href = "#" + ele.attr("data-id");
                        $("body").bind("mousewheel.myEvents", function () {
                            return false
                        });
                        setTimeout(function () {
            ...
            if (ele !== undefined) {
                        ...
                        if (vmc.attr("data-background-video") !== undefined) {
                            ...
                            if (bgVideopath !== vmc.attr("data-background-video")) {
                                ...
                                if (BigVid !== undefined) {
                                    ...
                                    if (vpp < 2)BigVid.show(vpp[0]); else BigVid.show(vpp[0], {altSource: vpp[1]})
                                }
                            } else if (BigVid !== undefined) {
                                $(".vidPlyPauBtn").data("view", true);
                                $(".vidPlyPauBtn").find("i").addClass("fa fa-eye-slash");
                                BigVid.getPlayer().play()
                            }
                            try {
                                if (BigVid !== undefined)BigVid.getPlayer().volume(videoVolume)
                            } catch (e) {
                            }
                        }
                    } else if (BigVid !== undefined) {
                ...
                if (ele.find(".big_video").length > 0)$("#big-video-wrap").css({"display": "block"})
            }
            ...
            if (self.curP.data("carouseGallLoad") ===
                ...
                self.curP.find(".carousel_container").each(function () {
                    ...
                    if ($(this).attr("data-link") === undefined)$(this).find(".carousel_thumbails").elastislide({minItems: 1}); else carousel_gallery_int($(this));
            ...
            if (self.alignPgHor) {
                ...
                self.clearInter2 =
                    setInterval(function () {
                        clearInterval(self.clearInter2);
                        if (!lowResDesktop)if (self.curP.hasClass("autoFullscreen"))self.siteFooter(true); else self.siteFooter()
                    }, 1500)
            }
            if (self.url !== self.curPgChk) {
                if (self.alignPgHor)self.htmlBody.scrollTop(0);
                try {
                    self.curP.find(".carousel_container").each(function () {
                        var mc = $(this);
                        ...
                        } else mc.find(".carousel_thumbails").each(function () {
                            $(this).data("fn")($(this).children().eq(0),
                                0, true)
                        })
                    })
                } catch (e) {
            ...
            if (!self.alignPgHor) {
                if (self.curP.hasClass("bodyBackground")) {
                    ...
                    if (img !== undefined)if (img !== "none")$("body").addClass("addBackground").css({"background-image": "url(" + img + ")"}); else $("body").removeClass("addBackground").css({"background-image": "none"})
                }
                self.beforePageLoad(self.curP)
            }
            if (!self.curP.data("linkBugFix")) {
                self.curP.data("linkBugFix",
                    true);
                $(".thumbItem_holder .thumbItem").find("a").bind("click", function (event) {
                    aLink = $(this);
                    if (aLink.attr("href") !== undefined && !aLink.hasClass("magnificPopup") && !aLink.hasClass("smoothPageLoad"))if (aLink.attr("_target") !== undefined)window.open(aLink.attr("href"), aLink.attr("target")); else window.location.href = aLink.attr("href");
                    return false
                })
            }
            ...
            var self = this;
            if (self.alignPgHor) {
                if (!isTouch)self.htmlBody.css({"overflow": "hidden"}); else self.htmlBody.css({"overflow": "auto"});
            ...
            for (var ik = 0; ik < self.navArry.length; ik++)if (self.navArry[ik].attr("data-id") === self.url)self.curP = self.navArry[ik];
            var isInCont = undefined;
            for (var ab = 0; ab < self.conArry.length; ab++)if (self.conArry[ab].attr("data-id") ===
                self.url)isInCont = self.conArry[ab];
            self.pgAll.stop();
            setTimeout(function () {
                ...
                if (isInCont !== undefined)isInCont.find("[data-animated-in]").each(function () {
                    if (lowResDesktop)self.animateObject($(this), 1E6); else self.animateObject($(this), self.scrollPos)
                })
            }, 500);
            if (self.superSlider && !supersizedOnBody)if (self.url ===
                self.homePg)setTimeout(function () {
                if ($.supersized.vars.is_paused)api.playToggle()
            }, 1E3); else setTimeout(function () {
                if (!$.supersized.vars.is_paused)api.playToggle()
            }, 1E3);
            if (self.rsSlider)if (self.url === self.homePg)setTimeout(function () {
                apiRS.revresume()
            }, 1E3); else setTimeout(function () {
                apiRS.revpause()
            }, 1E3);
            ...
            var sped2 = scrollPos < posT + self.headHig - 10 && scrollPos > 0 ? 0 :
                self.aniSpeed;
            if (self.alignPgHor)self.load_plugin_Items(self.curP);
            if (self.alignPgHor) {
                self.pgAll.stop();
                self.moveItem.stop();
                if (isInCont !== undefined)if (self.scrollHorizontal)self.pageCurPos = -Math.round(isInCont.position().left); else self.pageCurPos = -Math.round(isInCont.position().top);
                if (!lowResDesktop) {
                    if (self.superSlider && !supersizedOnBody)$(".supersized_gallery").css({
                        "visibility": "visible",
                        "display": "block"
                    });
                    if (!self.rez) {
                        self.pgAll.stop().scrollTop(posT);
                        if (isInCont !== undefined)if (self.pageCurPos <=
                            0)if (self.scrollHorizontal) {
                            var sped = self.moveItem.position().left >= self.pageCurPos - 5 && self.moveItem.position().left <= self.pageCurPos + 5 ? 0 : 1E3;
                            self.moveItem.stop()[animateSyntax]({"left": self.pageCurPos + "px"}, sped, "easeInOutQuart", function () {
                                if (posT > 10 && !supersizedOnBody)$(".supersized_gallery").css({
                                    "visibility": "hidden",
                                    "display": "none"
                                });
                                self.pageUpdate()
                            ...
                            var sped = self.moveItem.position().top >= self.pageCurPos - 5 && self.moveItem.position().top <= self.pageCurPos + 5 ? 0 : 1E3;
                            self.moveItem.stop()[animateSyntax]({
                                "top": self.pageCurPos +
                                "px"
                            }, sped, "easeInOutQuart", function () {
                                if (posT > 10 && !supersizedOnBody)$(".supersized_gallery").css({
                                    "visibility": "hidden",
                                    "display": "none"
                                });
                        ...
                        self.pgAll.stop().scrollTop(posT);
                        if (isInCont !== undefined)if (self.pageCurPos <= 0) {
                            if (self.scrollHorizontal) {
                                var sped = self.moveItem.position().left >= self.pageCurPos - 5 && self.moveItem.position().left <= self.pageCurPos + 5 ? 0 : 1E3;
                                self.moveItem.stop().css({"left": self.pageCurPos + "px"})
                            } else {
                                var sped = self.moveItem.position().top >= self.pageCurPos -
                                5 && self.moveItem.position().top <= self.pageCurPos + 5 ? 0 : 1E3;
                    ...
                    if (isInCont !== undefined)if (self.pageCurPos <= 0)if (!self.rez && (ipadDevice || !isTouch) && self.url !== self.curPgChk) {
                        for (var ik = 0; ik < self.navArry.length; ik++)if (self.navArry[ik].attr("data-id") === self.url) {
                        ...
                        if (self.scrollHorizontal) {
                            self.moveItem.css({
                                "left": self.pageCurPos + self.stageWidth * self.ir + "px",
                            ...
                            self.moveItem.css({
                                "top": self.pageCurPos + self.stageWidth * self.ir + "px",
                                "height": self.winHeight,
                                "overflow-y": "hidden"
                            });
                            self.moveItem.stop()[animateSyntax]({
                        ...
                        if (self.scrollHorizontal)self.moveItem.stop().css({"left": self.pageCurPos + "px"}); else self.moveItem.stop().css({"top": self.pageCurPos + "px"});
                        self.pageUpdate()
                    }
                    if ("!home" !== self.url) {
                ...
                if (self.onePage)self.autoPagePos(isInCont);
                self.pageUpdate()
            }
            ...
            self.scrollObj.stop(false).animate({scrollTop: posT}, self.aniSpeed, "easeInOutQuart", function () {
                if (self.enableLowPerformance)self.removeAutoposition = self.enableLowPerformance ? true : false
            })
        }, pageUpdate: function () {
            var self = this;
            for (var ik = 0; ik < self.navArry.length; ik++)if (self.navArry[ik].attr("data-id") === self.url)self.updatePage(self.curP);
            self.scroll_update()
        },
        page_setup: function () {
            var self = this;
            ...
            self.enableLowPerformance = isTouch || window.innerWidth < 1025;
            self.HideUnuse =
                self.alignPgHor ? false : self.enableLowPerformance ? true : false;
            if (!self.HideUnuse && window.innerWidth > 768)for (var bb = 0; bb < self.conArry.length; bb++) {
                self.conArry[bb].css({"visibility": "visible"});
                self.conArry[bb].css({"display": "block"});
                if (!self.conArry[bb].data("loaded"))self.load_plugin_Items(self.conArry[bb])
            }
            $("body").data("bgType", isMobileChk);
            if (self.alignPgHor) {
                if (!self.miniMenu)if (self.stageWidth < 1025)self.headerMc.addClass("mini"); else self.headerMc.removeClass("mini");
                if (!lowResDesktop)self.pgAll.css({
                    "height": self.StgHig,
                    "overflow": "hidden"
                }); else {
            ...
            self.headHig = self.stageWidth > 991 ? self.stageWidth < 1025 || !self.headMeuTyp1 ? pageHeaderHeight_mini : pageHeaderHeight : self.alignPgHor ? 0 : pageHeaderHeight_mini;
            if (self.headHig > 0)$(".mobile_topSpc").removeClass("removeSpc"); else $(".mobile_topSpc").addClass("removeSpc");
            $(".header_content").data("open", false);
            if (self.stageWidth > 991)$(".header_content").css({"display": "block"}); else $(".header_content").css({"display": "none"});
            ...
            self.bdy.find("img").each(function () {
                ...
                if ($(this).attr("data-src-small"))if (!mobVer || !$(this).attr("data-src-small")) {
                    var img_Src = $(this).data("src").split(".");
                    var iimg = $(this).attr("data-retina") === "yes" && retinaDevice ?
                    img_Src[0] + "@2x." + img_Src[1] : $(this).data("src");
                    if (String($(this).attr("src")) !== iimg) {
                        $(this).attr("src", iimg);
                        $(this).data("i_src", $(this).data("src"))
                    }
                } else if ($(this).attr("data-src-small")) {
            ...
            $("body").find(".parallax").each(function () {
                ...
                if (img !== undefined && !thbg.hasClass("bodyBackground") && img !== thbg.data("imgPath"))if (img !== "none") {
            ...
            $("body").find(".addVideo.backGroundVideo").each(function () {
                ...
                if (img !== "none" || img !== undefined)vid2.css({
                    "background-image": "url(" +
                    img + ")"
                }); else vid2.css({"background-image": "none"})
            });
            var tppp = 0;
            ...
            $("body").find(".carousel_preview.fullScreenGallery_items, .carousel_preview.fullScreenGallery_items .carousel_item").each(function () {
                if (self.alignPgHor)$(this).css({
                    "height": self.StgHig,
                    "width": "100%"
                }); else $(this).css({"height": self.StgHig - self.headHig, "width": "100%"})
            });
            var dimSiz = self.stageWidth > self.stageHeight ||
            self.mobile ? self.winHeight : self.winWidth;
            ...
            if (isTouch)$(".overlayPattern").hide();
            if (BigVid !== undefined)$("#big-video-wrap").css({"height": $("#big-video-wrap").parent().parent().height()});
            if (self.IEbrowser || !self.supportScrollBar)self.htmlBody.css({"overflow": "auto"})
        }, beforePageLoad: function (ele) {
            var self = this;
            var isInCont = undefined;
            for (var ab = 0; ab < self.conArry.length; ab++)if (self.conArry[ab].attr("data-id") === self.url) {
                ...
                try {
                    self.conArry[ab].find(".flexslider").each(function () {
                        ...
                        if (fc.data("loadInPop") === undefined && fc.data("slid") !== undefined &&
                            fc.data("autPly"))fc.data("slid").resume()
                    })
                } catch (e) {
                }
            } else try {
                self.conArry[ab].find(".flexslider").each(function () {
                    if ($(this).data("slid") !== undefined)$(this).data("slid").pause()
                })
            } catch (e) {
            }
            if (!ele.hasClass("portfolioPage"))ele.find(".flexSlideshow").each(function () {
                try {
                    if (!$(this).data("loaded")) {
            ...
            for (var ik = 0; ik < ele.data("fms").length; ik++)if (ele.data("fms")[ik].data("loaded")); else {
            ...
            var self = this;
            $("body").find(".parallax").each(function () {
                var img = !isMobileChk ? $(this).attr("data-src") :
                    $(this).attr("data-src-small") ? $(this).attr("data-src-small") : $(this).attr("data-src");
                var imgAtt = !isTouch ? "scroll" : "scroll";
                var vd = false;
                var thbg = $(this);
                thbg.find(".backGroundVideo").each(function () {
            ...
            if (self.stageWidth <= 991) {
                ...
                setTimeout(function () {
                    $(".header .header_content ul li ul").removeClass("hideIt")
                }, 1200)
            }
            var jjj = false;
            self.pgNexPre.removeClass("hideBtn");
            for (var ik = 0; ik < self.navArry.length; ik++)if (self.navArry[ik].attr("data-id") === self.url) {
                if (self.navArry[ik].hasClass("removeNexPrevBtn"))self.pgNexPre.addClass("hideBtn");
            ...
            if (self.HideUnuse)for (var bb = 0; bb < self.conArry.length; bb++)if (beforePg + 1 !== bb && beforePg - 1 !== bb && beforePg !== bb || beforePg + 1 === self.conArry.length + 1) {
                if (self.conArry[bb].css("visibility") !== "hidden")self.conArry[bb].css({"visibility": "hidden"})
            } else if (self.conArry[bb].css("visibility") !== "visible")self.conArry[bb].css({"visibility": "visible"});
            for (var ab = 0; ab < self.conArry.length; ab++)if (self.conArry[ab].attr("data-id") !== self.url)for (var ik =
                0; ik < self.conArry[ab].data("fms").length; ik++)if (self.conArry[ab].data("fms")[ik].data("loaded"))self.conArry[ab].data("fms")[ik].fmMainSlider("pause_slideshow");
            $("body").find(".addVideo").each(function () {
                $(this).data("isPlaying", false)
            });
            if ($("body").find(".mfp-wrap").length > 0)try {
                $.magnificPopup.close()
            } catch (e) {
            }
            if (BigVid !== undefined)BigVid.getPlayer().pause();
            $("#big-video-vid").css({"display": "none"});
            if (isInCont !== undefined)isInCont.find(".big_video").each(function () {
                var vmc = $(this);
                if (vmc.attr("data-background-video") !==
                    undefined) {
                    vmc.append($("#big-video-wrap"));
                    $("#big-video-vid").css({"display": "block"});
                    var videoVolume = isNaN(vmc.attr("data-video-volume")) ? defaultVolume : Number(vmc.attr("data-video-volume"));
            ...
            if (self.alignPgHor) {
                if (isInCont !== undefined)if (isInCont.hasClass("bodyBackground")) {
                    var img = !isMobileChk ? isInCont.attr("data-src") : isInCont.attr("data-src-small") ? isInCont.attr("data-src-small") :
                        isInCont.attr("data-src");
                    var imgAtt = !isTouch ? "fixed" : "scroll";
                ...
                if (self.curPg === "") {
                    ...
                    if (self.pgSub ===
                        undefined && self.onePage)window.location.href = "" + self.url;
            ...
            /*if (self.prePg === self.curPg) {
                ...
                if (self.curPg === "") {
                    self.curPg = self.prePg = self.url;
                    if (self.pgSub === undefined && self.onePage)window.location.href = "#" + self.url;
                    self.cM = $('a[href$="#' + self.curPg + '"]').parent();
                    if (self.alignPgHor)self.pgAll.stop().animate({scrollTop: "0px"}, 0, "easeInOutQuart")
                } else {
                    self.curPg = self.url;
                    var pagScrl_Speed = window.pageYOffset !== 0 ? self.aniSpeed :
                        50;
                    var con_Speed = 0;
                    if (self.prePg !== self.url) {
                        if (self.alignPgHor)self.pgAll.stop().animate({scrollTop: "0px"}, pagScrl_Speed, "easeInOutQuart", function () {
                        })
                    } else {
                        if (isInCont !== undefined)self.page_position();
            ...
            var self = this;
            $("body").find(".masonry_items").each(function () {
                self.masonNum = self.stageWidth < 1149 ?
                    self.mobile ? self.midMobile ? 2 : 1 : 3 : self.stageWidth > 1480 ? $(this).hasClass("max_col4") ? 4 : 5 : 4;
            ...
            if (obj.parent().hasClass("imgBorder"))lodr = !lodr ? obj.parent().parent().hasClass("fixedHeight") : lodr;
            var cc = obj.attr("class");
            var st = obj.attr("style");
            var $img;
            if (st)$img = $('<img class="' + cc + ' style="' + st + '" />'); else $img = $('<img class="' + cc + '" />');
            ...
            if (lodr)$img.attr("src", imSrc).load(function () {
                $(this).parent().find(".loading_x").remove();
                if ($(this).hasClass("resize_align"))self.resizeImg($(this));
                if (!$(this).hasClass("noSelfAnimate"))$(this).show().addClass(aniInEff); else $(this).show()
            }).error(function () {
                $(this).parent().find(".loading_x").remove()
            }).each(function () {
                if (this.complete)$(this).trigger("load")
            ...
            (function ($) {
                ...
                function loadContent(hash2) {
                    ...
                    if (hash !== "") {
                        if (origContent ===
                            "" && self.curPg === "")origContent = $('.contentWrapper [data-id="' + "#" + self.homePg + '"]');
                        if (self.hisPath !== hash) {
                            self.hisPath = hash;
                            self.page_load(hash)
                        } else if (self.pgSub !== undefined && self.projFm) {
                    ...
                    } else if (!self.onePage)if (self.pgSub !==
                        undefined && self.projFm) {
                        p2 = self.pgSub.split("=");
                        if (Number(p2[1]) !== self.curSlide) {
                            self.curFmSlider = $(".pageHolder .fmSlides").data("sArry");
                            self.curSlide = Number(p2[1]);
        ...
        }, animateObject: function (mc,
                                    scrlPos) {
            var self = this;
            if (self.disableAnimation)return;
            var mcPos = Math.round(mc.position().top);
            if (mc.data("isSliderObj") || mc.data("isDisplay") || mc.data("caro_prev"))return;
            if (mc.data("isMasonry") && self.alignPgHor)return;
            var par = 0;
            if (mc.attr("data-anchor-to") !== undefined) {
                var mc2 = mc;
                for (var io = 0; io < mc.attr("data-anchor-to").split(".").length; io++)mc2 = mc2.parent();
                par = mc2.position().top
            }
            if (self.alignPgHor) {
                if (mcPos + par - (self.winHeight - (self.winHeight / 4 - 100)) > Math.abs(scrlPos) && self.headHig !== 0)return
            } else {
                var mcPos =
                    Math.round(mcPos + mc.data("main_holder").position().top);
                if (mcPos + par - (self.winHeight - (self.winHeight / 4 - 100)) > Math.abs(scrlPos) && self.headHig !== 0)return
            }
            mc.data("isDisplay", true);
            if (mc.data("isAniObj") && !isTouch)self.animate_objectBeforeDisplay(mc);
            if (mc.attr("data-animated-innerContent") === "yes") {
                ...
                mc.children().each(function () {
                    var mc2 = $(this);
                    mc2.stop();
                    var aniTyp = mc.attr("data-animated-in") !== undefined ? mc.attr("data-animated-in") :
                        "animated fadeIn";
                    mc.data("in", aniTyp);
                    ...
                    setTimeout(function () {
                        if (cssAnimate)mc2.css({"visibility": "visible"}).removeClass(aniTyp).addClass(aniTyp).one("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend", function () {
                            $(this).removeClass(mc.data("in"));
                            if (mc.data("isAniObj") && !isTouch)self.animate_objectAfterDisplay($(this))
                        }); else {
                ...
                setTimeout(function () {
                    if (cssAnimate)mc.css({"visibility": "visible"}).removeClass(aniTyp).addClass(aniTyp).one("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend",
                        function () {
                        ...
                        mc.css({
                            ...
                            "opacity": 0,
                            "top": tp + 15
                        }).removeClass(aniTyp).animate({"opacity": 1, "top": tp}, 350, "easeInOutQuad", function () {
            ...
            if (self.knobAni)try {
                obj.find(".animate_counter").each(function () {
                    var selK = $(this);
                    if (selK.hasClass("knob"))selK.val(0).trigger("change");
            ...
            if (self.knobAni)try {
                obj.find(".animate_counter").each(function (i) {
                    ...
                    $(selK.data("ani")).animate({"top": selK.data("val")}, {
                        step: function (now, fx) {
                            if (selK.hasClass("knob"))selK.val(now).trigger("change");
                            if (selK.data("display"))selK.data("display").text(Math.round(now))
                ...
                $(this).find(".video_hover").each(function () {
                    ...
                    vv.bind(eventMc, function (event) {
                        ...
                        vid.find("img").fadeOut(100, function () {
                            ...
                            vid_.children(":first-child").embedPlayer(vid_.attr("data-url"), vid_.width() + "px",
            ...
            try {
                $("body").find(".addVideo").each(function () {
                    if (!$(this).hasClass("backGroundVideo") && !$(this).data("isPlaying") || $(this).data("url_") !== self.curP.attr("data-id"))self.video_delete($(this));
            ...
            try {
                ...
                $("body").find(".addVideo.backGroundVideo").each(function () {
                    var vid_ = $(this);
                    if (vid_.data("url_") !== self.curP.attr("data-id"))self.video_delete(vid_)
                })
            } catch (e) {
            }
        },
        video_delete: function (mc) {
            ...
            mc.find(".vid").each(function () {
                ...
                try {
                    if ($(this).length > 0)jQuery("#" + $(this).children(":first-child").attr("id")).tubeplayer("destroy")
                } catch (e) {
                }
                $(this).remove()
            });
            ...
            $("body").find(".m-Scrollbar").each(function () {
                $(this).mCustomScrollbar({
                    ...
                    callbacks: {
                        onScroll: function () {
                            if (mcs.top < -150) {
                                for (var ik = 0; ik < $(this).data("fms").length; ik++)if ($(this).data("fms")[ik].data("loaded"))$(this).data("fms")[ik].fmMainSlider("pause_slideshow");
                                self.curPageShow.find("[data-animated-in]").each(function () {
                                    var tps = mcs.topPct < 90 ? mcs.top : 1E6;
            ...
            $("body").find(".scroll-pane").each(function () {
                self.apis.push($(this).jScrollPane({
            ...
            $(".mCSB_scrollTools").bind("mouseleave.scrolEvent", function () {
                if (!self.scrlPress)$(this).find(".mCSB_dragger_bar")[animateSyntax]({"width": "2px"}, 300, "easeInOutQuart")
            });
            $(".mCSB_scrollTools .mCSB_dragger_bar").bind("mousedown.scrolEvent", function () {
                self.scrlPress = true
            });
            $("body").bind("mouseup.scrolEven",
                function () {
            ...
            if (self.stageWidth > 991)if (self.alignPgHor) {
                if (self.stageWidth <= 991)self.curP.css({"height": "auto"});
                try {
                    if (self.curP.hasClass("m-Scrollbar")) {
                        self.curP.mCustomScrollbar("update");
                        if (rPos !== undefined)self.curP.mCustomScrollbar("scrollTo", "top")
                    }
                    self.curP.find(".m-Scrollbar").each(function () {
                        $(this).mCustomScrollbar("update");
                        if (rPos !== undefined)$(this).mCustomScrollbar("scrollTo", "top")
                    })
                } catch (e) {
                }
                try {
                    if (!isTouch)if (rePos !== 0 && rePos !== undefined) {
                        self.curP.mCustomScrollbar("update");
                ...
                if (rPos !== undefined)self.scrollObj.stop(false).animate({scrollTop: rePos + "px"}, 500, "easeInOutQuart")
            } else if (!isTouch && !self.IEbrowser && self.supportScrollBar) {
                self.nicScrl.resize();
                if (rPos !== undefined)self.scrollObj.stop(false).animate({scrollTop: rePos + "px"}, 500, "easeInOutQuart")
            }
            ...
            e.find("li").each(function () {
                ...
                $(this).each(function () {
                    $(this).children(":first-child").css("width",
                        "0px");
            ...
            if (colr.split("orange").length > 1)self.curTempColor = "-orange"; else if (colr.split("red").length > 1)self.curTempColor = "-red";
            self.setting_tool = $(".setting_tools");
            setPreviewBtn();
            $(".setting_tools .iButton").click(function () {
                if (self.setting_tool.hasClass("hideTool"))self.setting_tool.removeClass("hideTool");
                else self.setting_tool.addClass("hideTool")
            });
            $(".mUp").click(function () {
            ...
            $(".colNight").click(function () {
                if (!$(this).hasClass("active")) {
                    $("#set_color").attr("href",
                        "css/color-night" + self.curTempColor + ".css");
            ...
            if ($(".header .container").length > 0) {
                $(".mType1").click(function () {
                    if ($(".header .container").length >
                        ...
                        var mp = Math.round(Math.abs(100 / $(".header_content>ul").children().length)) + .9;
                ...
                $(".mType2").click(function () {
                    if ($(".header .container").length > 0) {
                        ...
                        self.headMeuTyp1 =
                            self.headerMc.hasClass("menuType1");
                        self.page_setup()
                    }
                    setPreviewBtn()
                })
            } else $(".mType1, .mType2").css({"opacity": ".5"});
            ...
            function setPreviewBtn() {
                ...
                if (cUrl.split("orange").length >
                    1)$(".temHigLight2").addClass("active"); else if (cUrl.split("red").length > 1)$(".temHigLight3").addClass("active"); else $(".temHigLight1").addClass("active");
                if (self.stageWidth < 1025 || self.miniMenu)self.headerMc.addClass("mini");
                if (!self.miniMenu)if (self.stageWidth > 1024)if (self.scrollPos > 150 && !self.headerMc.hasClass("menuType1"))self.headerMc.addClass("mini"); else self.headerMc.removeClass("mini"); else self.headerMc.addClass("mini")
            }
        }
    };
    $.fn.mainFm = function (params) {
        ...
        } else if (instance[params])return instance[params].apply(instance, Array.prototype.slice.call(arguments, 1))
    }
})(jQuery);
